...
namespace TunNetCom.SilkRoadErp.Sales.Api.Features.DeliveryNote.ExportToExcel;

public class ExportDeliveryNotesToExcelEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        _ = app.MapGet("/api/delivery-notes/export/excel", HandleExportToExcelAsync)
    ...
    public static async Task<Results<FileContentHttpResult, StatusCodeHttpResult>> HandleExportToExcelAsync(
        ...
        [FromServices] ILogger<ExportDeliveryNotesToExcelEndpoint> logger,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            logger.LogInformation(
                "ExportDeliveryNotesToExcelEndpoint called with startDate: {StartDate}, endDate: {EndDate}, customerId: {CustomerId}, technicianId: {TechnicianId}, tagIds: {TagIds}, status: {Status}",
            ...
            logger.LogInformation("Exporting {Count} delivery notes to Excel with {ColumnCount} columns", deliveryNoteList.Count, columnsToExport.Count);
            ...
            logger.LogInformation("VAT totals calculated - Base7: {Base7}, Base13: {Base13}, Base19: {Base19}, Vat7: {Vat7}, Vat13: {Vat13}, Vat19: {Vat19}", 
            ...
            var filename = $"BonsLivraison_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        {
            logger.LogError(ex, "Error exporting delivery notes to Excel format");
