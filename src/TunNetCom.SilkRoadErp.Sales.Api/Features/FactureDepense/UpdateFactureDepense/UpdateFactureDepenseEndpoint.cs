...
public class UpdateFactureDepenseEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        {
            var command = new UpdateFactureDepenseCommand(id, request.Date, request.Description ?? string.Empty, request.MontantTotal, request.LignesTVA ?? new(), request.DocumentBase64);
