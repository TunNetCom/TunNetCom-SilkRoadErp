...
public class UpdateAvoirFournisseurValidator : AbstractValidator<UpdateAvoirFournisseurCommand>
{
    public UpdateAvoirFournisseurValidator()
    {
        _ = RuleFor(x => x.Num)
        ...
        _ = RuleForEach(x => x.Lines)
            .SetValidator(new AvoirFournisseurLineValidator());
    }
}

public class AvoirFournisseurLineValidator : AbstractValidator<AvoirFournisseurLineRequest>
{
    public AvoirFournisseurLineValidator()
    {
        _ = RuleFor(x => x.RefProduit)
            .NotEmpty().WithMessage("ref_produit_is_required");

        _ = RuleFor(x => x.DesignationLi)
            .NotEmpty().WithMessage("designation_li_is_required");
