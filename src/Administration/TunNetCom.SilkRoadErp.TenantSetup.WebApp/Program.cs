...
// Prefer Aspire-assigned URLs over config so we don't force 5003/5007 when running under AppHost (avoids "address already in use")
...
if (string.IsNullOrEmpty(aspireUrls) || aspireUrls.Contains("5001"))
    Environment.SetEnvironmentVariable("ASPNETCORE_URLS", "https://localhost:5003;http://localhost:5007");
...
if (string.IsNullOrWhiteSpace(urls) || urls.Contains("5001"))
    urls = "https://localhost:5003;http://localhost:5007";
