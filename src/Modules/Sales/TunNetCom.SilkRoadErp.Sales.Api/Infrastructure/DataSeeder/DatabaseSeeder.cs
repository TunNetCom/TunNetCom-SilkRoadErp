...
public class DatabaseSeeder
{
    ...
    private async Task SeedAccountingYearAsync(SalesContext context)
    {
        ...
        _logger.LogInformation("Création de l'exercice comptable 2026 (actif)...");
        ...
        if (activeYears.Any())
        {
            ...
            foreach (var year in activeYears) year.SetInactive();
            await context.SaveChangesAsync();
        }

        var existing2026 = await context.AccountingYear.FirstOrDefaultAsync(ay => ay.Year == 2026);
        if (existing2026 != null)
        {
            _logger.LogInformation("L'exercice comptable 2026 existe déjà. Activation de cet exercice...");
            existing2026.SetActive();
            await context.SaveChangesAsync();
            _logger.LogInformation("✓ Exercice comptable 2026 activé avec succès.");
            return;
        }

        var accountingYear2026 = AccountingYear.CreateAccountingYear(2026, isActive: true);

        _logger.LogInformation("Ajout de l'exercice comptable 2026 à la base de données...");
        try
        {
            await context.AccountingYear.AddAsync(accountingYear2026);
            var saved = await context.SaveChangesAsync();
            _logger.LogInformation("✓ Exercice comptable 2026 (actif) inséré avec succès. {Saved} changements sauvegardés.", saved);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "✗ ERREUR lors de l'insertion de l'exercice comptable 2026: {Message}", ex.Message);
            throw;
        }
    }
